...
<div class="max-w-5xl mx-auto space-y-8">
        ...
        <div class="grid gap-6 md:grid-cols-3">
            ...
            <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition duration-300 group">
                <div class="flex items-center mb-3">
                    <span class="bg-blue-100 text-blue-800 text-xs font-bold px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">INFO</span>
                    <span class="text-slate-400 text-xs ml-auto">{{ $info->created_at->diffForHumans() }}</span>
                </div>
                <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2 line-clamp-1">{{ $info->judul }}</h3>
                <p class="text-slate-600 dark:text-slate-300 text-sm line-clamp-2">{{ $info->isi }}</p>
            </div>
            @endforeach
        </div>
    </section>
    ...
    <div class="grid md:grid-cols-2 gap-8">
        ...
        <!-- CARD 1: BIODATA DIRI -->
        <!-- Tambahkan opacity jika belum aktif -->
        <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 flex flex-col h-full relative overflow-hidden transition {{ !Auth::user()->is_active ? 'opacity-75 grayscale-[0.5]' : '' }}">
            ...
            <div class="flex items-center justify-between mb-6 relative z-10">
                ...
                <!-- Status Badge -->
                @if(!Auth::user()->is_active)
                    <span class="px-3 py-1 text-xs font-bold rounded-full bg-red-100 text-red-600 border border-red-200">NON-AKTIF</span>
            ...
            @elseif(Auth::user()->calonMahasiswa)
                ...
                <div class="space-y-3 text-sm text-slate-600 dark:text-slate-300 mb-8 flex-grow relative z-10 bg-slate-50 dark:bg-slate-700/50 p-4 rounded-lg">
                    <div class="flex justify-between"><span class="text-slate-500">NISN:</span> <span class="font-semibold">{{ Auth::user()->calonMahasiswa->nisn }}</span></div>
                    <div class="flex justify-between"><span class="text-slate-500">Prodi:</span> <span class="font-semibold">{{ Auth::user()->calonMahasiswa->prodi_pilihan }}</span></div>
                </div>
        ...
        <!-- CARD 2: PEMBAYARAN -->
        <!-- Ini tetap dikunci sampai biodata diisi -->
        <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 flex flex-col h-full relative overflow-hidden transition opacity-60">
            <div class="flex items-center justify-between mb-6 relative z-10">
                ...
                @else
                     <span class="px-3 py-1 text-xs font-bold rounded-full bg-slate-100 text-slate-600">TERKUNCI</span>
                @endif
            </div>

            @if(Auth::user()->calonMahasiswa && Auth::user()->is_active)
                @if(Auth::user()->calonMahasiswa->pembayaran)
                    <button disabled class="w-full py-2.5 bg-slate-100 text-slate-500 rounded-xl cursor-not-allowed">Bukti Terkirim</button>
                @else
                    <p class="text-slate-500 mb-8">Lakukan pembayaran biaya pendaftaran.</p>
                    <a href="{{ route('pembayaran.create') }}" class="block w-full py-3 bg-green-600 text-white text-center rounded-xl shadow-lg">Upload Bukti Bayar</a>
                @endif
            @else
                <p class="text-slate-500 dark:text-slate-400 mb-8 flex-grow relative z-10">
                    @if(!Auth::user()->is_active)
